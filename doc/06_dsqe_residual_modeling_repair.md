...
点只有在距离某个 actor 中心不超过保守 footprint 半径且 actor 轨迹有效时才获得 actor 角色；无法匹配的动态点 `role_valid=False`。明确静态类点角色为 0 并保持有效。没有 actor 元数据时，动态类别不会伪造为有效动态 GT。
...
结果：`28 passed`。测试覆盖 BaseLine 残差恒等性、零初始化梯度、逐时刻语义独立性、运动状态角色匹配、混合 Query 点级路由、动态损失软死区、重复位移防止、TASS 固定和旧 state dict 兼容；同时通过已有 DSQE 两步 CUDA smoke test。
